...
function match_pattern(rule, date) {
    ...
    if (rule.weekday &&
        !rule.weekday[0].includes(date.weekday)) return false

    if (rule.month &&
        !rule.month[0].includes(date.month)) return false

    if (rule.monthday &&
        !rule.monthday.includes(date.monthday.start) &&
        !rule.monthday.includes(date.monthday.end)) return false

    if (rule.monthweek &&
        !rule.monthweek.includes(date.monthweek.start) &&
        !rule.monthweek.includes(date.monthweek.end)) return false


    if (rule.date_list &&
        !rule.date_list.includes(date.date)) return false
...
function parse_date(date) {
    ...
    if (d.weekday == 0) { d.weekday = "sS" } // If it is Sunday, show Sunday (S) and Sunday vigils (s) 
    if (d.weekday == 6) { d.weekday = "6s" } // If it is Saturday, show Saturday (6) and Sunday vigils (s)
