...
function date_satisfies_rule(pDate, rule) {
    ...
    if (rule.monthday) {
        if (!rule.monthday.includes(pDate.monthday.start) &&
            !rule.monthday.includes(pDate.monthday.end)) return false
    }
    if (rule.monthweek) {
        if (!rule.monthweek.includes(pDate.monthweek.start) &&
            !rule.monthweek.includes(pDate.monthweek.end)) return false
    }
    if (rule.date_list) {
...
function parse_date(date) {
    ...
    var pDate = {
        ...
        monthweek: { // eg. the first Thursday of the month has monthweek.start = 1, the last one has monthweek.end = 1
    ...
    if (pDate.weekday == 0) { pDate.weekday = "S" } 
    // TODO? 
